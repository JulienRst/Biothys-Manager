<?php
	...
	//////////////////////////////////////////////
	$typedocument = 4; // Confirmation de commande
	//////////////////////////////////////////////
	...
	function generateIdDoc($type,$company,$order){
		...
		$order->id_delivery_order = '6'.$year.$country.$id_to_print;
		return $id_document;
	}
	...
	class PDF extends FPDF {
		...
		function SecondTable($order){
			...
			$count = 1;
			$this->Cell(185,6,'Delivery order number : '.$order->id_delivery_order,1,0,'C');
			$this->Ln();
	...
	$table_first = array(array(utf8_decode('n° :'),$id_document,date('d.m.Y',$order->getDate_billing())),array('UST-ID :',$company->getUst_id(),utf8_decode($employee->getSurname().' '.$employee->getName())),array('Your ID :',$company->getId(),'0'.'/{nb}'));
	...
	$pdf->Cell(180,15,utf8_decode('Invoice'),0,1,'R',false);
	...
	if($order->getIs_factorem() == "yes"){
		checkIfDropPage($pdf,$order,$table_first,50);
		///////////////////////////////////////////////////////////////////////
		//Septième bloc : Text Factorem
		$pdf->setY($pdf->GetY() + 5);
		$pdf->SetFont('Arial','',8);
		$pdf->SetTextColor(0);
		$pdf->MultiCell($pdf->w - 25,3,utf8_decode($text_factorem));
	}
